...
static void readFromStdin(int epoll_fd, int client_fd);
...
int main() {
    ...
    struct epoll_event ev = buildEvent(EPOLLIN | EPOLLET, socket_server_fd);
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, socket_server_fd, &ev) < 0) {
        die("EPOLL_CTL_ADD failed");
    }

    makeSocketNonBlocking(0); // stdin   
    struct epoll_event eventStdin = buildEvent(EPOLLIN | EPOLLET, STDIN_FILENO);
    if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, STDIN_FILENO, &eventStdin) < 0) {
        die("EPOLL_CTL_ADD failed");
    }
    ...
    while (!sigintReceived) {
        ...
        for (i = 0; i < num_ready; i++) {
            if (events[i].data.fd == socket_server_fd) {
                ...
                readFromStdin(epoll_fd,events[i].data.fd);
