...
// ./server -v --module-dir modules --address localhost --port 4001
...
void server_run(struct in_addr local_address, uint16_t port) {
    ...
    while (1) {
        ...
        /* We have a connection.  Print a message if we're running in
           verbose mode.  */
        if (verbose) {
            socklen_t address_length_v = {0};

            /* Get the remote address of the connection.  */
            address_length_v = sizeof(socket_address);
            rval = getpeername(connection, &socket_address, &address_length_v);
            assert(rval == 0);
            /* Print a message.  */
